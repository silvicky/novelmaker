...
import io.silvicky.novel.compiler.types.StructType;
...
public class StructDeclaration extends NonTerminal implements ASTNode
{
    ...
    public void travel()
    {
        if(declarationBlock!=null)
        {
            ...
            structType=new StructType(declarationBlock.fields);
            if(name!=null)
            {
                //TODO really?
                if(ctx==-1)registerStruct(name,structType);
                else registerLocalStruct(name,structType);
            }
        }
        else structType=lookupStruct(name);
    }
